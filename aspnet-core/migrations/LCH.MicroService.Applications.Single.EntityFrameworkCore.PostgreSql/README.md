...
2. 在菜单中选择 PostgreSQL 数据库上下文：
   ```
   [2] LCH.MicroService.Applications.Single.EntityFrameworkCore.PostgreSql
   ```
...
生成的 SQL 脚本将保存在：
```
aspnet-core/InitSql/LCH.MicroService.Applications.Single.EntityFrameworkCore.PostgreSql/
```
