...
2. 在菜单中选择 SQL Server 数据库上下文：
   ```
   [3] LCH.MicroService.Applications.Single.EntityFrameworkCore.SqlServer
   ```
...
生成的 SQL 脚本将保存在：
```
aspnet-core/InitSql/LCH.MicroService.Applications.Single.EntityFrameworkCore.SqlServer/
```
