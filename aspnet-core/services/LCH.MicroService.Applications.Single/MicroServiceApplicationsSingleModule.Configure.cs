...
namespace LCH.MicroService.Applications.Single;
...
public partial class MicroServiceApplicationsSingleModule
{
    ...
    private void ConfigureVirtualFileSystem()
    {
        ...
        {
            options.FileSets.AddEmbedded<MicroServiceApplicationsSingleModule>("LCH.MicroService.Applications.Single");
        });
    }
    ...
    private void ConfigureWrapper()
    {
        Configure<AbpWrapperOptions>(options =>
        {
            ...
            // 微信消息不能包装
            options.IgnoreNamespaces.Add("LCH.Abp.WeChat");
        });
    }
    ...
    private void ConfigureSecurity(IServiceCollection services, IConfiguration configuration, bool isDevelopment = false)
    {
        ...
        if (!isDevelopment)
        {
            ...
            services
                .AddDataProtection()
                .SetApplicationName("LCH.Abp.Application")
                .PersistKeysToStackExchangeRedis(redis, "LCH.Abp.Application:DataProtection:Protection-Keys");
        }
    ...
    private void ConfigureWeChat()
    {
        Configure<AbpWeChatMessageHandleOptions>(options =>
        {
            // 回复文本消息
            options.MapMessage<
                LCH.Abp.WeChat.Official.Messages.Models.TextMessage,
                LCH.MicroService.Applications.Single.WeChat.Official.Messages.TextMessageReplyContributor>();
            // 处理关注事件
            options.MapEvent<
                LCH.Abp.WeChat.Official.Messages.Models.UserSubscribeEvent,
                LCH.MicroService.Applications.Single.WeChat.Official.Messages.UserSubscribeEventContributor>();

            options.MapMessage<
                LCH.Abp.WeChat.Work.Common.Messages.Models.TextMessage,
                LCH.MicroService.Applications.Single.WeChat.Work.Messages.TextMessageReplyContributor>();
        });
    }
}
