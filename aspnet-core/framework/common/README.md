...
## 包含的项目列表
1. **LCH.Abp.Aliyun.Authorization**
   - 提供阿里云基础认证功能，支持AppKeyId和AccessKeySecret的配置。

2. **LCH.Abp.AspNetCore.HttpOverrides**
   - 实现HTTP传输标头的重写，支持获取反向代理中的真实客户地址。

3. **LCH.Abp.AspNetCore.Mvc.Client**
   - 提供可配置的用户配置缓存时间，支持多租户接口。

4. **LCH.Abp.BackgroundJobs.Hangfire**
   - 基于Hangfire实现的后台作业模块，支持即时、延迟和周期性任务。

5. **LCH.Abp.ExceptionHandling**
   - 提供统一的异常处理和通知机制，支持自定义异常处理程序。

6. **LCH.Abp.Location**
   - 提供地理编码、反向地理编码和IP地理位置解析功能。

7. **LCH.Abp.IdGenerator**
   - 实现分布式唯一ID生成器，支持雪花算法。
...
```csharp
[DependsOn(typeof(LCH.Abp.Aliyun.Authorization))]
