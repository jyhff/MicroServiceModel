...
```bash
# 安装微服务模板:lam
dotnet new install LCH.Abp.MicroService.Templates

# 安装单体应用模板:laa
dotnet new install LCH.Abp.AllInOne.Templates
```

### 安装 labp 命令行工具

```bash
  dotnet tool install --global LCH.Abp.Cli
```

### 创建项目

```bash
# 简写名称：laa (LCH Abp AllInOne)
...
3. 使用 IDE 打开 `LCH.MicroService.Applications.Single` 解决方案进行调试或发布
...
- `LCH.MicroService.Applications.Single/appsettings.Development.PostgreSql.json`
```json
{
  "ConnectionStrings": {
...
- `LCH.MicroService.Applications.Single/appsettings.Development.MySql.json`
```json
{
  "ConnectionStrings": {
...
- `LCH.MicroService.Applications.Single/appsettings.Development.SqlServer.json`
```json
{
  "ConnectionStrings": {
...
```json
{
  ...
  "Redis": {
    ...
    "InstanceName": "LCH.Abp.Application"
  },
  "Features": {
    "Validation": {
      "Redis": {
...
1. 选择启动配置
   - 选择 `LCH.MicroService.Applications.Single.DbMigrator/Properties/launchSettings.json` 中的 `Single.PostgreSql.Dev`

2. 配置数据初始化：

   - 修改 `LCH.MicroService.Applications.Single.DbMigrator/appsettings.PostgreSql.json` 中的数据库连接信息

3. 执行数据迁移：
   - 运行 `LCH.MicroService.Applications.Single.DbMigrator` 项目
   - 等待数据迁移完成，基础表数据将被初始化

### 服务启动
1. 选择启动配置
   - 选择 `LCH.MicroService.Applications.Single/Properties/launchSettings.json` 中的 `Single.PostgreSql.Dev`
1. 运行 `LCH.MicroService.Applications.Single` 项目
